...
res <- lfcShrink(DEseq2_object, coef = i, type = "apeglm", quiet = FALSE)
...
rld=df_rld)
# plot selection ----------------------------------------------------------
log2_plus_1_norm<-log2(counts(DEseq2_object, normalized=TRUE)[, 1:2]+1)

df <- bind_rows(
as_tibble(log2_plus_1_norm) %>% mutate(transformation = "log2(x + 1)"),
as_tibble(df_rld[, 1:2]) %>% mutate(transformation = "rlog"),
as_tibble(df_vsd[, 1:2]) %>% mutate(transformation = "vst"))
