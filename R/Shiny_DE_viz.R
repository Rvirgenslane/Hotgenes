...
Pheatmap_Viz<-function(Input=NULL,
hotList=NULL,samples_ids=1,readouts="rlog",ncut=NULL,
selected_contrast=2,padj_cut=0.1,lfc_cut=0){

if(readouts=="rlog"){
gene_levels<-Input$Normalized_Expression$rld
}
if(readouts=="vsd"){
gene_levels<-Input$Normalized_Expression$vsd
}

if(is.null(hotList)){
contrast_name<-names(Input$Output_DE[selected_contrast])
contrast_name_lists <- setNames(vector(length(contrast_name),
mode="list"), contrast_name)

for (i in contrast_name) {
sig_ids<-rownames(Input$Output_DE[[i]][Input$Output_DE[[i]]$padj<padj_cut&
abs(Input$Output_DE[[i]]$log2FoldChange)>=lfc_cut,])
...
# tab1 DE frames -----------------------------------------------------------
conditionalPanel('input.dataset === "FactoMiner_PCA"', # DEseq2_coefficients
...
tabPanel("FactoMiner_PCA", plotOutput(outputId = "PCA_plot"),
...
dm<-Pheatmap_Viz(Input = DEseq2_export,
# filename= "ph.pdf",
