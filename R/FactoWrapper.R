...
# PCA start
res<-PCA(dm_pheno, scale.unit=T, ncp=5,
...
axes = c(1,2), repel = T,  label=label_sel,
...
title = title,
addEllipses=T, ellipse.level=ellipse.level) + theme_classic() +
scale_shape_manual(values = rep(20,
length(res.hcpc$data.clust[,habillage_selection] )))
} else if(biplot==FALSE) {
res_PPI_pa_1<-fviz_pca_ind(res, axes = c(1,2), repel = T,  label=label_sel,
habillage=res.hcpc$data.clust[,habillage_selection] ,
col.quanti.sup = "red",
col.var = c("black"),
pointsize = pointsize, labelsize = labelsize,
